...
namespace MySql.Data.Common
{
  ...
  internal class SharedMemoryStream : Stream
  {
    ...
    public override void Close()
    {
      if (connectionClosed != null)
      {
        var isClosed = connectionClosed.WaitOne(0);
        if (!isClosed)
        {
    ...
    private void GetConnectNumber(uint timeOut)
    {
      ...
      catch (Exception)
      {
        ...
        var prefixedMemoryName = @"Global\" + memoryName;
        connectRequest =
    ...
    {
      var prefix = memoryName + "_" + connectNumber;
    ...
    public override int Read(byte[] buffer, int offset, int count)
    {
      ...
      var len = Math.Min(count, bytesLeft);
      var baseMem = data.View.ToInt64() + position;
    ...
    public override void Write(byte[] buffer, int offset, int count)
    {
      ...
      while (leftToDo > 0)
      {
        ...
        var baseMem = data.View.ToInt64() + 4;
